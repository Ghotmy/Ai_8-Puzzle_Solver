...
public class Solver {
    ...
    private class SearchNode implements Comparable <SearchNode> {
        ...
        public SearchNode(Board state, SearchNode parent, int moves, SearchTechnique st) {
            this.state = state;
            this.parent = parent;
            this.moves = moves;
            switch (st) {
                case MANHATTAN:
                    ...
                    break;
                case EUCLIDEAN:
                    this.score = state.euclidean() + moves;
                    break;
                case BFS:
                    this.score = moves;
                    break;
    ...
    public Solver(Board initial, SearchTechnique st) {
        ...
        if (solvable) {
            ...
            pq.insert(new SearchNode(initial, null, 0, st));
            ...
            while (!pq.isEmpty()) {
                ...
                if(curr.state.isGoal()) {
                    ...
                    for (Board neighbor : curr.state.neighbors()) {
                        ...
                        pq.insert(new SearchNode(neighbor, curr, curr.moves + 1, st));
                    }
                }
            }
